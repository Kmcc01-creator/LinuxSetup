...
def test_api_logger():
    ...
    with patch('logging.getLogger') as mock_get_logger:
        ...
        # Verify that info was called twice (before and after function execution)
        assert mock_logger.info.call_count == 2
        mock_logger.info.assert_any_call("Calling test_function with args=('test_param',), kwargs={}")
        mock_logger.info.assert_any_call("test_function completed successfully")
...
def test_api_logger_error():
    ...
    with patch('logging.getLogger') as mock_get_logger:
        ...
        assert "Error in error_function: Test error" in args[0]
        assert kwargs.get('exc_info') is True

def test_invalid_format():
    """Test invalid documentation format handling."""
    with pytest.raises(ValueError, match="Invalid documentation format: invalid"):
        DocGenerator("invalid")
